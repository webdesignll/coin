...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MXC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("mxcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MXC unit input in lnReqAmount, we generate the URI with MXC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MXC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
